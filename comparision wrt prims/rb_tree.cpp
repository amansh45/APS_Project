...
rbNode::rbNode(int x, int y, rbNode *par) {
	vid=y;
	val=x; 
...
void RBTree::insert(int n, int vid, rbNode *parent) { 
	rbNode *newNode = new rbNode(n, vid, parent); 
	if (root == NULL) { 
		newNode->color = BLACK; 
		root = newNode; 
	} else { 
...
rbNode *RBTree::returnMin(rbNode *x) {
	if(x->left==NULL)
		return x;
	else
		return returnMin(x->left);
}

rbNode *RBTree::retrieveMinRB() {
	if(root == NULL)
		return NULL;
	else
...
rbNode * RBTree::search_helper(rbNode *root, int vertex) {
	if(root == NULL)
		return NULL;
	if(root->vid == vertex)
		return root;
	else {
		rbNode *t1 = search_helper(root->left, vertex);
		if(t1!=NULL)
			return t1;
		else {
			return search_helper(root->right, vertex);
		}
	}
}

rbNode * RBTree::AVLpreOrderSearch(int vertex) {
	return search_helper(root, vertex);
}
